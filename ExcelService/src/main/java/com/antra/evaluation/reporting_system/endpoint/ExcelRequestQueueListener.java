...
//        log.info("Get request: {}", request);
        ...
        try {
            if(request.getHeaders()==null){
                excelService.deleteFile(request.getData().get(0).get(0));
            }else{
                String fileId = excelService.findFileIdByReqId(request.getReqId());
                if(fileId!=null){
                    file = excelService.updateFile(request, fileId);
                }else {
