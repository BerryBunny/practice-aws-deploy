...
public class ExcelGenerationController {
    ...
    @PutMapping("/excel/{id}")
    public ResponseEntity<ExcelResponse> updateExcel(@PathVariable String id,
                                                     HttpEntity<ExcelRequest> httpEntity) throws FileNotFoundException {
        ...
        log.debug("Got Request to Update Single Sheet Excel:{}", request);
        ExcelFile fileInfo = excelService.updateFile(request, id);
        ExcelResponse response = new ExcelResponse();
        BeanUtils.copyProperties(fileInfo, response);
        response.setFileDownloadLink(this.generateFileDownloadLink(fileInfo.getFileId()));
        response.setFileLocation(fileInfo.getFileLocation());
