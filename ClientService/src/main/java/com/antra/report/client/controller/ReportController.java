...
import com.antra.report.client.pojo.request.ReportDeletionRequest;
...
public class ReportController {
    ...
    @DeleteMapping("/report/{id}/sync")
    public ResponseEntity<GeneralResponse> deleteReportDirectly(@PathVariable String id,
                                                                @RequestBody @Validated ReportDeletionRequest request) {
        ReportDeletionVO requestVO = new ReportDeletionVO();
        BeanUtils.copyProperties(request, requestVO);
        requestVO.setReqId(id);
        log.info("Got Request to delete report - sync: {}", requestVO);
        return ResponseEntity.ok(new GeneralResponse(reportService.deleteReportsSync(requestVO)));
    }

    @DeleteMapping("/report/{id}/async")
    public ResponseEntity<GeneralResponse> deleteReportAsync(@PathVariable String id,
                                                             @RequestBody @Validated ReportDeletionRequest request) {
        ReportDeletionVO requestVO = new ReportDeletionVO();
        BeanUtils.copyProperties(request, requestVO);
        requestVO.setReqId(id);
        log.info("Got Request to delete report - async: {}", requestVO);
        reportService.deleteReportsAsync(requestVO);
        return ResponseEntity.ok(new GeneralResponse());
    }
